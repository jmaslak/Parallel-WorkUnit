...
Because information passed back from the asyncronous processes is sent
serialized and sent through a pipe, it is not possible at this time to
return all the rich data structures Perl has. In particular, you cannot
return C<REGEXP>, C<CODE>, or C<OBJECT> references (note that C<OBJECT>
should not be confused with Perl objects created without the "class"
keyword introduced experimentally in Perl 5.38).
