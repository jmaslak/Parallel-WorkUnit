...
# This tests the Parallel::WorkUnit functionality with IO::Async
# It is essentially a copy of 01-Basic except using the IO::Async system.
...
SKIP: {
    ...
    my $wu = Parallel::WorkUnit->new( { use_io_async => $loop } );
    ...
    for ( 0 .. $PROCS - 1 ) {
        my $v = $_;
        $future[$v] = $loop->new_future();
        $wu->async( sub { return $v; }, sub { $future[$v]->done; cb(@_) } );
        is( $wu->count, 1 + $v, 1 + $v . " workers are executing [FUTURE]" );
    }

    for ( 0 .. $PROCS - 1 ) {
        $future[$_]->get;
        ok( exists( $RESULTS{$_} ), "Worker First Exec $_ returned properly [FUTURE]" );
    }
    is( $wu->count, 0, "no processes running after condvar wait [FUTURE]" );
    ...
    my $wu2 = Parallel::WorkUnit->new( use_io_async => $loop );
