...
# Make sure we can use the ordered method
ok( !$wu->ordered, "Ordered initializes to false" );
$wu->ordered(1);
ok( $wu->ordered, "Ordered set to true" );
$wu->ordered(undef);
ok( !$wu->ordered, "Ordered reset to false" );

# We're going to spawn 10 children and test the return value
# We also test setting ordered with units running
my %RESULTS;
my $PROCS = 10;
for ( 0 .. $PROCS - 1 ) {
    $wu->async( sub { return $_; }, \&cb );
    is( $wu->count, 1 + $_, 1 + $_ . " workers are executing" );
    dies_ok { $wu->ordered(1); } "Can't set ordered to 1 when workers running";
}
...
my $pid = $wu->async( sub { return 'HERE'; }, \&cb );
